...
readonly session_name="${RTI_MQTTSHAPES_TMUX_SESSION:-MQTT+JSON DEMO}"
...
function send_tmux_commands {
    ...
    tmux send-keys -t "$session_name:1.2" \
        "sleep 4 && DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            ...
            "$NDDSHOME/bin/rtiroutingservice -cfgFile ${DEMO_DIR}/etc/shapesdemo_mqtt_json.xml -cfgName shapesdemo_bridge" C-m

    tmux send-keys -t "$session_name:1.0" \
        "sleep 2 && ${DEMO_DIR}/scripts/shapes_mqtt_publisher.sh GREEN shapes/square" C-m

    tmux send-keys -t "$session_name:1.1" \
        "sleep 2 && ${DEMO_DIR}/scripts/shapes_mqtt_subscriber.sh shapes/square" C-m
