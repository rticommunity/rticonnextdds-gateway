...
readonly session_name="${RTI_MQTTSHAPES_TMUX_SESSION:-MQTT+PB DEMO}"
...
function send_tmux_commands {
    ...
    tmux send-keys -t "$session_name:1.2" \
        "sleep 4 && DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            ...
            "$NDDSHOME/bin/rtiroutingservice -cfgFile ${DEMO_DIR}/etc/shapesdemo_mqtt_protobuf.xml -cfgName shapesdemo_bridge" C-m

    tmux send-keys -t "$session_name:1.0" \
        "sleep 2 && DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            "LD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            "${DEMO_DIR}/bin/shapes_mqtt_publisher square-pub tcp://127.0.0.1:1883 GREEN shapes/square 1" C-m

    tmux send-keys -t "$session_name:1.1" \
        "sleep 2 && DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            "LD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            "${DEMO_DIR}/bin/shapes_mqtt_subscriber square-sub tcp://127.0.0.1:1883 shapes/square 1" C-m
