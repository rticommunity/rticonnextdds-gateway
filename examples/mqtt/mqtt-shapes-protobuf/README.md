...
adapter plugin, and the Protobuf transformation plugin.

The MQTT plugin establishes a client connection to the MQTT broker, while
the Protobuf transformation is used to manipulate the binary payload contained in
each MQTT message.
...
A Protobuf type equivalent to `ShapeType` will be used to manipulate the
binary payload of each MQTT message:
...
- **MQTT Publisher**: publishes a random green square every second.
  ...
  the *MQTT Broker* on MQTT topic `shapes/square`.
...
4. Publish some MQTT squares using the provided script `shapes_mqtt_publisher`:
   ...
   ```sh
   bin/shapes_mqtt_publisher GREEN shapes/square
   ```

5. Susbcribe to MQTT squares using the provide script `shapes_mqtt_subscriber`:

   ```sh
   bin/shapes_mqtt_subscriber shapes/square
   ```
...
7. Start RTI Routing Service with the bridging configuration:
   ...
   ```sh
   rtiroutingservice -cfgFile etc/shapesdemo_mqtt_protobuf.xml -cfgName shapesdemo_bridge
   ```
...
4. Use the provided script `tmux_session.sh` to start a session with all other commands:
