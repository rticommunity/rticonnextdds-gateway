...
: "${RTI_MQTT_SHAPES_PROTOBUF:=}"

#== initialization
set -o nounset

readonly session_name="${RTI_MQTTSHAPES_TMUX_SESSION:-MQTT DEMO}"
...
function send_tmux_commands {
    ...
    tmux send-keys -t "$session_name:1.0" \
        "DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            "LD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH ./mqtt-shapes $(
        if [ -n "${RTI_MQTT_SHAPES_PROTOBUF}" ]; then
            echo "-protobuf"
        fi
    )" C-m
    ...
    tmux send-keys -t "$session_name:1.1" \
        "sleep 4 && DYLD_LIBRARY_PATH=${DEMO_DIR}/../../../lib:$NDDSHOME/lib/$CONNEXTDDS_ARCH " \
            ...
            "$NDDSHOME/bin/rtiroutingservice -cfgFile ${DEMO_DIR}/etc/shapes_bridge$(
        if [ -n "${RTI_MQTT_SHAPES_PROTOBUF}" ]; then
            echo "_protobuf"
        fi        
    ).xml -cfgName shapes_bridge" C-m

    [ -n "${RTI_MQTT_SHAPES_PROTOBUF}" ] ||
