...
int ShapesAgent_initialize(
    struct ShapesAgent *self,
    const enum ShapeAgentFormat format)
{
    int rc = 1;
    ...
    *str_ref = DDS_String_dup(SHAPES_AGENT_PROFILES);
    ...
    switch (self->format) {
        case SHAPES_AGENT_FORMAT_JSON:
        {
            const DDS_UnsignedLong msg_buf_len = (DDS_UnsignedLong)
...
int ShapesAgent_publish_message(struct ShapesAgent *self)
{
    ...
    switch (self->format) {
        ...
        case SHAPES_AGENT_FORMAT_PROTOBUF:
        {
            if (DDS_RETCODE_OK
                != ShapeType_protobuf_serialize(
                        ...
                        size)) {
                RTI_MQTT_ERROR_1(
                        "failed to serialize protobuf shape",
                        "(%d)",
                        self->msg_sent_tot)
                goto done;
            }
        ...
        DDS_Boolean in)
{
    ...
    if (msg_payload_len == 0) {
        ...
        switch (self->format) {
            ...
            {
                if (DDS_RETCODE_OK
                    != ShapesAgent_deserialize_shape_json(
                            msg_payload,
                            msg_payload_len,
                            self->shape)) {
            ...
            case SHAPES_AGENT_FORMAT_PROTOBUF:
            {
                if (DDS_RETCODE_OK
                    != ShapeType_protobuf_deserialize(
                            ...
                            msg_payload_len,
                            self->shape)) {
                    RTI_MQTT_ERROR("failed to deserialize Protobuf shape")
                    goto done;
                }
                break;
            }
        ...
        if (DDS_RETCODE_OK
            != DDS_DynamicData_get_long(
                    ...
                    "x",
                    DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED)) {
            RTI_MQTT_ERROR("failed to get 'x' field")
            goto done;
        }
        ...
        if (DDS_RETCODE_OK
            != DDS_DynamicData_get_long(
                    ...
                    "y",
                    DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED)) {
            RTI_MQTT_ERROR("failed to get 'y' field")
            goto done;
        }
        ...
        if (DDS_RETCODE_OK
            != DDS_DynamicData_get_long(
                    ...
                    "shapesize",
                    DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED)) {
            RTI_MQTT_ERROR("failed to get 'shapesize' field")
            goto done;
        }
    ...
    if (msg_payload_len > 0) {
        printf("color='%s' x=%d y=%d shapesize=%d", color, x, y, size);
    }
...
DDS_ReturnCode_t ShapesAgent_deserialize_shape_json(
