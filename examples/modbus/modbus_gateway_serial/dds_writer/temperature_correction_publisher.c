...
int publisher_main(int domainId, int sample_count)
{
    ...
    topic = DDS_DomainParticipant_create_topic(
        participant, "Modbus Serial Temp Correction",
    ...
    temperature_correction_writer = temperature_correctionDataWriter_narrow(writer);
    ...
    instance = temperature_correctionTypeSupport_create_data_ex(DDS_BOOLEAN_TRUE);
    if (instance == NULL) {
        fprintf(stderr, "temperature_correctionTypeSupport_create_data error\n");
        publisher_shutdown(participant);
        return -1;
    }
    ...
    */
    /* Delete data sample */
    retcode = temperature_correctionTypeSupport_delete_data_ex(instance, DDS_BOOLEAN_TRUE);
    if (retcode != DDS_RETCODE_OK) {
        fprintf(stderr, "temperature_correctionTypeSupport_delete_data error %d\n", retcode);
    }
