...
void MBus_WTH_CO2_LCD_ETH_INPUTListener_on_data_available(
    ...
    DDS_DataReader* reader)
{
    MBus_WTH_CO2_LCD_ETH_INPUTDataReader *MBus_WTH_CO2_LCD_ETH_INPUT_reader = NULL;
    struct MBus_WTH_CO2_LCD_ETH_INPUTSeq data_seq = DDS_SEQUENCE_INITIALIZER;
    struct DDS_SampleInfoSeq info_seq = DDS_SEQUENCE_INITIALIZER;
    DDS_ReturnCode_t retcode;
    int i;

    MBus_WTH_CO2_LCD_ETH_INPUT_reader = MBus_WTH_CO2_LCD_ETH_INPUTDataReader_narrow(reader);
...
int subscriber_main(int domainId, int sample_count)
{
    ...
    retcode = MBus_WTH_CO2_LCD_ETH_INPUTTypeSupport_register_type(participant, type_name);
    if (retcode != DDS_RETCODE_OK) {
    ...
    topic = DDS_DomainParticipant_create_topic(
        participant, "Example StreamReader",
