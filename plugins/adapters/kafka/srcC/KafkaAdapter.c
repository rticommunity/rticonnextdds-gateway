    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    if (adapter == NULL)
    {
        //RTI_RoutingServiceEnvironment_set_error(env, "RTI_RS_KafkaAdpterPlugin memory allocation error");
        return NULL;
    }
    ...
    adapter->base.adapter_plugin_delete =
        RTI_RS_KafkaAdapterPlugin_delete;
    ...
    adapter->base.connection_create_session =
        NULL;
    adapter->base.connection_delete_session =
        NULL;
    ...
    adapter->base.stream_writer_write =
        RTI_RS_KafkaStreamWriter_write;

    adapter->base.stream_reader_read =
        RTI_RS_KafkaStreamReader_read;
    adapter->base.stream_reader_return_loan =
        RTI_RS_KafkaStreamReader_return_loan;
    adapter->base.stream_reader_update =
        NULL;
    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    if (connection == NULL)
    {
        //RTI_RoutingServiceEnvironment_set_error(env, "RTI_RS_KafkaConnection memory allocation error");
        return NULL;
    }

    connection->adapter = (struct RTI_RS_KafkaAdapterPlugin *)adapter;

    connection->bootstrap_servers = RTI_RoutingServiceProperties_lookup_property(
        properties, "bootstrap.servers");
    if (connection->bootstrap_servers == NULL)
    {
        //RTI_RoutingServiceEnvironment_set_error(env, "bootstrap.servers missing property brokers");
        return NULL;
    }
