...
MariaDBOdbcTestHandler::MariaDBOdbcTestHandler(
...
void MariaDBOdbcTestHandler::connect()
{
...
void MariaDBOdbcTestHandler::disconnect()
{
    odbc_conn_->disconnect_from_database();
}

unsigned long long MariaDBOdbcTestHandler::getRowCount(
...
void MariaDBOdbcTestHandler::executeQuery(const std::string& query)
{
    SQLHSTMT stmt = odbc_conn_->create_statement();
...
std::vector<int> MariaDBOdbcTestHandler::coordinatesX(const std::string& tableName)
{
    std::vector<int> coordinates;
    
    // Query to get x coordinates from JSON column, ordered by RTI_HISTORY_ORDER
    std::string query = "SELECT JSON_EXTRACT(json_sample, '$.coordinates.x') FROM " + 
                        query_converter_->quotes() + tableName + query_converter_->quotes() + 
...
std::vector<std::string> MariaDBOdbcTestHandler::listTables()
{
    // Query all user tables from the configured test schema.
    std::string query = "SELECT table_name FROM information_schema.tables "
                        "WHERE table_schema = '" + database_name_ + "'";
...
void MariaDBOdbcTestHandler::drop_all_tables()
{
