...
dds::topic::Topic<::RTI::MQTT::KeyedMessage>
MqttMessageTester::create_topic_in()
{
    return dds::topic::Topic<::RTI::MQTT::KeyedMessage>(
        dds_participant_,
        "test_messages_in");
}

dds::topic::Topic<::RTI::MQTT::KeyedMessage>
MqttMessageTester::create_topic_out()
{
    return dds::topic::Topic<::RTI::MQTT::KeyedMessage>(
        dds_participant_,
        "test_messages_out");
...
dds::pub::DataWriter<::RTI::MQTT::KeyedMessage>
MqttMessageTester::create_writer()
{
    return dds::pub::DataWriter<::RTI::MQTT::KeyedMessage>(
        dds_publisher_, dds_topic_in_);
}

dds::sub::DataReader<::RTI::MQTT::KeyedMessage>
MqttMessageTester::create_reader()
{
    return dds::sub::DataReader<::RTI::MQTT::KeyedMessage>(
        dds_subscriber_, dds_topic_out_);
}

void
MqttMessageTester::run()
{
    ...
    //    - Prepare mqtt data
    //    - Write message
    //    - Wait to receive a message back from RS.
    //    - Deserialize and validate message
    std::string message;
    ::RTI::MQTT::KeyedMessage mqtt_message;
    ...
    for (int32_t count = 0; count < max_count; count++) {
        ...
        // 2.1 Prepare MQTT message data
        message = "Test message " + std::to_string(count + 1);
        std::vector<uint8_t> & mqtt_data = mqtt_message.payload().data();
        mqtt_data.resize(message.size());
        std::copy(message.begin(), message.end(), mqtt_data.begin());

        // 2.3 Write message
        dds_writer_.write(mqtt_message);
        ...
        dds::sub::LoanedSamples<::RTI::MQTT::KeyedMessage> samples = dds_reader_.take();
        std::cout
            << "[" << tester_id_ << "] "
        ...
        if (samples[0].info().valid()) {
            // Deserialize and validate samples[0].data()
            const std::vector<uint8_t> & received_data = samples[0].data().payload().data();
            ...
            std::cout
                << "[" << tester_id_ << "] "
                << "Message " << (count + 1) << " OK: '" << received_message << "'" << std::endl;
        } else {
...
int main(int argc, char *argv[])
{
    ...
    return tester_main<MqttMessageTester>(argc, argv);
}
