...
int process_data(dds::sub::DataReader<MyTypeWithSequences> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples<MyTypeWithSequences> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::sub::DataReader<MyTypeWithSequences> reader(subscriber, topic);
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        ...
        std::cout << "MyTypeWithSequences subscriber sleeping up to 1 sec..." << std::endl;
