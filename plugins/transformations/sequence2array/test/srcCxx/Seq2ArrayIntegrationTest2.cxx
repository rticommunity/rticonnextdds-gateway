...
class Seq2ArrayIntegrationTest2 : public Seq2ArrayTester<
    ::test::inttest2::PubType,
    ::test::inttest2::SubType> {
public:
    Seq2ArrayIntegrationTest2(const int32_t tester_id)
    : Seq2ArrayTester(tester_id, "Seq2ArrayIntTest2Array", "Seq2ArrayIntTest2Seq")
    {}

protected:
    void fill_message(::test::inttest2::PubType& message, const int32_t count) override {
        using namespace ::test;

        TestStruct struct_val;
        this->fill_test_struct(struct_val, count);

        TestUnionArray union_val;
        this->fill_test_union_array(union_val, count);

        // fill in member1
        std::array<TestStruct, 5> member1;
        member1[0] = struct_val;
        member1[1] = struct_val;
        member1[2] = struct_val;

        message.member1(member1);

        // fill in member2
        std::array<TestEnum, 5> member2;
        // When built in Debug mode, some compilers do not initialize the entire array
        // so we explicitly fill it with a default value first
        std::fill(member2.begin(), member2.end(), TestEnum::TEST_ENUM_VALUE_1);
        member2[0] = struct_val.member4();
        member2[1] = struct_val.member4();
        member2[2] = struct_val.member4();

        message.member2(member2);

        // fill in member3
        std::array<TestUnionArray, 5> member3;
        member3[0] = union_val;
        member3[1] = union_val;
        member3[2] = union_val;
        ...
        // fill in member4
        std::vector<TestStruct> member4;
        member4.push_back(struct_val);
        member4.push_back(struct_val);
    ...
    void verify_message(const ::test::inttest2::SubType& message, const int32_t count) override {
        using namespace ::test;
        ...
        if (message.member2().size() != 5) {
            std::cerr <<
                "[" << tester_id_ << "] INVALID VALUE RECEIVED " <<
                "count=" << count << ", "
                "member2 " <<
                "expected size=5, " <<
                "found size=" << message.member2().size() << std::endl;
        ...
        for (size_t i = 0; i < 5; i++) {
            const TestUnionSeq& member3 = message.member3()[i];
            if (i < 3) {
                TestEnum expected_d;
                switch(count % 3) {
                    case 0:
                ...
                switch(count % 3) {
                    ...
                    case 1:
                        ...
                        break;
                    default:
                        if (member3.member3().size() != 5) {
                            std::cerr <<
                                "[" << tester_id_ << "] INVALID VALUE RECEIVED " <<
                                "count=" << count << ", "
                                "member3[" << i << "].member3 " <<
                                "expected size=5, " <<
                                "found size=" << member3.member3().size() << std::endl;
                            throw std::runtime_error("invalid size for member3.member3");
                        }
        ...
        if (message.member4().size() != 2) {
            std::cerr <<
                "[" << tester_id_ << "] INVALID VALUE RECEIVED " <<
                "count=" << count << ", "
                "member4 " <<
                "expected size=2, " <<
                "found size=" << message.member4().size() << std::endl;
            throw std::runtime_error("invalid size for member4");
        }
...
int main(int argc, char *argv[])
{
    ...
    return tester_main<Seq2ArrayIntegrationTest2>(argc, argv);
}
