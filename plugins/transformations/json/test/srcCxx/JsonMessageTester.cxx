...
JsonMessageTester::create_topic_in()
{
...
dds::topic::Topic<::test::SerializedMessage>
JsonMessageTester::create_topic_out()
{
    return dds_topic_in_;
}

dds::pub::DataWriter<::test::SerializedMessage>
JsonMessageTester::create_writer()
{
...
JsonMessageTester::create_reader()
{
...
void
JsonMessageTester::serialize_message(
    const ::test::Message& message,
    ::test::SerializedMessage& serialized_message)
{
    serialized_message.src(message.src());
    std::string msg_json = rti::topic::to_string(message, rti::topic::PrintFormatProperty::Json());
    std::vector<uint8_t> & msg_data = serialized_message.data();
    msg_data.resize(msg_json.size());
    std::copy(msg_json.begin(), msg_json.end(), msg_data.begin());
}

void
JsonMessageTester::deserialize_message(
    const ::test::SerializedMessage& serialized_message,
    ::test::Message& message)
{
    const std::vector<uint8_t> & serialized_data = serialized_message.data();
    std::string msg_json(serialized_data.begin(), serialized_data.end());
    message = rti::topic::from_string<::test::Message>(msg_json, rti::topic::PrintFormatKind::JSON);
}

void
JsonMessageTester::run()
{
    ...
    for (int32_t count = 0; count < max_count; count++) {
        ...
        message.src(tester_id_);
        message.count(count);
        message.message("Test message " + std::to_string(count + 1));
        ...
        if (samples[0].info().valid()) {
            deserialize_message(samples[0].data(), received_message);
            if (received_message.src() != tester_id_) {
                throw std::runtime_error("Received message with unexpected src");
            }
            if (received_message.count() != count) {
                throw std::runtime_error("Received message with unexpected count");
            }
            if (received_message.message() != message.message()) {
                throw std::runtime_error("Received message with unexpected message");
            }
        } else {
...
int main(int argc, char *argv[])
{
    ...
    return tester_main<JsonMessageTester>(argc, argv);
}
