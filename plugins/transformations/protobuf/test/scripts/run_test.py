...
msg_repeater = TestProcess(
    ["tsfm_protobuf_repeater", "--id", 2, "--domain", args.support_domain_id],
    ignore_rc=True)

# Routing Service process
routing_service = TestCase.routingservice("tsfm_protobuf_test.xml")

# Plugin tester process
tester = TestCase.tester("tsfm_protobuf_tester", 1, args.domain_id)
...
TestCase.run(test,
    ...
    qos_profile="tsfm_protobuf_test_qos.xml"
)
